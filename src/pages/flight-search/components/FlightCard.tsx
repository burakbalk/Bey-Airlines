...
export default function FlightCard({ flight }: FlightCardProps) {
  ...
  return (
    <div className={`rounded-2xl p-6 shadow-md hover:shadow-lg transition-all border-l-4 border border-gray-100 ${isVip ? 'bg-amber-50 border-l-amber-500' : 'bg-white border-l-primary'}`}>
      {/* Class badge */}
      ...
      <div className="grid grid-cols-12 gap-4 items-center">
        ...
        <div className="col-span-3">
          ...
          <p className="text-base font-semibold text-primary mt-1">{flight.departure.airport}</p>
        ...
        <div className="col-span-3 flex flex-col items-center">
          ...
          <div className="w-full flex items-center">
            ...
            <div className="w-7 h-7 flex items-center justify-center mx-2 bg-primary/10 rounded-full">
              <i className="ri-plane-line text-base text-primary"></i>
            </div>
            <div className="flex-1 border-t-2 border-dashed border-gray-300"></div>
          </div>
          <span className="mt-2 px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700 whitespace-nowrap">
            {flight.type}
          </span>
        </div>
        ...
        <div className="col-span-3">
          ...
          <p className="text-base font-semibold text-primary mt-1">{flight.arrival.airport}</p>
          <p className="text-sm text-gray-400">{flight.arrival.city}</p>
        </div>

        {/* Price & CTA */}
        <div className="col-span-3 text-right">
          <p className="text-3xl font-bold text-primary">{flight.price.toLocaleString('tr-TR')} TL</p>
          ...
          <button 
            ...
            className={`w-full mt-3 font-semibold py-3 rounded-xl transition-colors whitespace-nowrap cursor-pointer text-sm ${isVip ? 'bg-amber-500 hover:bg-amber-600 text-white' : 'bg-primary hover:bg-red-700 text-white'}`}
          >
            Seç
          </button>
        </div>
